...
3. **Add GitHub secrets**:
    ...
    | `TF_BUCKET_PREFIX` | Bucket prefix/path (e.g., `cloudflare-turnstile`) |

4. **Run the workflow**:
    - **Apply**: Go to Actions → **CD - Cloudflare Turnstile (Apply)** → fill in all inputs
    - **Destroy**: Go to Actions → **CD - Cloudflare Turnstile (Destroy)** → fill in essential inputs
